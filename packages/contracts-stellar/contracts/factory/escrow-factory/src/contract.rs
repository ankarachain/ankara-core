...
enum DataKey {
    ...
    FeeRecipient,
    AllDeployed,
    DeployerEscrows(Address),
}

/// Mirrors `EscrowFactory.sol` — a single implementation slot rather than
/// the `Template` enum + mapping the multi-template factories use, since
/// this factory only ever deploys one contract type (`MilestoneEscrow`).
#[contract]
...
impl EscrowFactory {
    ...
    pub fn set_implementation(env: Env, wasm: soroban_sdk::Bytes) -> BytesN<32> {
    ...
    fn record_deployment(env: &Env, deployer: &Address, address: &Address) {
        ...
        let key = DataKey::DeployerEscrows(deployer.clone());
    ...
    pub fn total_deployed(env: Env) -> u32 {
        Self::all_deployed_escrows(env).len()
    }

    pub fn all_deployed_escrows(env: Env) -> Vec<Address> {
        env.storage()
            .instance()
            .get(&DataKey::AllDeployed)
            .unwrap_or_else(|| Vec::new(&env))
    ...
    pub fn get_deployer_escrows(env: Env, deployer: Address) -> Vec<Address> {
        env.storage()
            .persistent()
            .get(&DataKey::DeployerEscrows(deployer))
            .unwrap_or_else(|| Vec::new(&env))
    }

    /// Mirrors `EscrowFactory.sol`'s `deployEscrow(...)`.
    ...
    pub fn deploy_escrow(
        ...
        let args: Vec<Val> = soroban_sdk::vec![
            ...
            payer.into_val(&env),
            payee.into_val(&env),
            arbiter.into_val(&env),
            escrow_token.into_val(&env),
            identity_verifier.into_val(&env),
            timelock_duration.into_val(&env),
            amounts.into_val(&env),
            description_hashes.into_val(&env),
        ];
        ...
        env.events().publish(
            ...
            (deployed_address.clone(), deployer, payer, payee),
        );
        deployed_address
    }
}
