...
use crate::metadata::{self, MiningRightsNFTMetadata};

#[contract]
pub struct MiningRightsNFT;

#[contractimpl]
impl MiningRightsNFT {
    pub fn initialize(
        env: Env,
        asset_id: BytesN<32>,
        country_code: String,
    ...
    pub fn mint(env: Env, to: Address, meta: MiningRightsNFTMetadata) -> u64 {
    ...
    pub fn is_license_expired(env: Env, token_id: u64) -> bool {
        metadata::is_license_expired(&env, token_id)
    }
    ...
    pub fn update_metadata(env: Env, token_id: u64, meta: MiningRightsNFTMetadata) {
    ...
    pub fn renew_license(env: Env, token_id: u64, new_expiry: u64) {
        require_role(&env, Role::Manager);
        let (old, version) = metadata::renew_license(&env, token_id, new_expiry);
        env.events()
            .publish((symbol_short!("renewed"), token_id), (old, new_expiry));
        env.events()
            .publish((symbol_short!("meta_upd"), token_id), version);
    }
