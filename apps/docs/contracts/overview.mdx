...
├── contracts/
│   ├── base/
│   │   ├── AnkaraChainBaseToken.sol      ← ERC-20 UUPS base (this page)
│   │   ├── AnkaraNFTBase.sol             ← ERC-721 UUPS base, see NFT Layer
│   │   ├── AnkaraMultiToken.sol          ← ERC-1155 UUPS base, see Multi-Token Layer
│   │   └── ManualOracle.sol              ← Reference price oracle
│   ├── interfaces/
│   │   ├── IIdentityVerifier.sol
│   │   ├── IAnkaraNFT.sol
│   │   ├── IAnkaraMultiToken.sol
│   │   └── IAnkaraOracle.sol
│   ├── templates/
│   │   ├── FarmlandToken.sol             ┐
│   │   ├── CommodityReceiptToken.sol     │ ERC-20 (6 templates)
│   │   ├── RealEstateToken.sol           │
│   │   ├── InvoiceToken.sol              │
│   │   ├── CarbonCreditToken.sol         │
│   │   ├── MiningRightsToken.sol         ┘
│   │   ├── FarmlandNFT.sol               ┐
│   │   ├── RealEstateNFT.sol             │ ERC-721 (4 templates)
│   │   ├── MiningRightsNFT.sol           │
│   │   ├── CommodityVaultNFT.sol         ┘
│   │   ├── CommodityBatchToken.sol       ┐ ERC-1155 (2 templates)
│   │   └── PoolVault.sol                 ┘
│   ├── escrow/
│   │   └── MilestoneEscrow.sol           ← Tranche-based escrow, see Escrow
│   └── factory/
│       ├── TokenFactory.sol              ← ERC-20 deploys
│       ├── NFTFactory.sol                ← ERC-721 deploys
│       ├── MultiTokenFactory.sol         ← ERC-1155 deploys
│       ├── EscrowFactory.sol             ← Escrow deploys
│       └── AnkaraFactoryRegistry.sol     ← Unified registry across all 4 factories
└── typechain-types/                       ← Auto-generated, gitignored
```

This page covers the ERC-20 base layer and the shared UUPS/factory patterns that every other layer follows. See [NFT Layer](/contracts/nft-layer), [Multi-Token Layer](/contracts/multi-token-layer), and [Escrow](/contracts/escrow) for the layer-specific details.
...
```typescript
// In tests
const verifier = await WhitelistVerifier.deploy(admin.address);
