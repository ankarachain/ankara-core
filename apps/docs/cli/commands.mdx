...
## Payments Commands (Stellar)

## `ankara batch-pay`

Pay many recipients from a CSV file in as few transactions as possible, using a deployed
`batch-disburser` contract (100 recipients per transaction). See
[Payments](/contracts/payments).

```bash
ankara batch-pay [--contract <address>] [--token <address>] [--file payouts.csv] [--reference <text>] [-y]
```

The CSV has one `recipient,amount` per line (amount as an integer in the token's smallest
unit). A header row and `#` comment lines are allowed:

```csv
recipient,amount
GA...WORKER1,1500000000
GB...WORKER2,2200000000
```

Each transaction is atomic. If any recipient in a batch fails (for example, not verified
by the token's identity verifier), that whole batch reverts. Pass `--yes` to skip the
