...
`nftFactoryAddress`, `multiTokenFactoryAddress`, and `escrowFactoryAddress` are only required for the commands in their respective sections above (NFT, multi-token/pool, and escrow commands) — `ankara init` only writes `factoryAddress` by default.
